...
public class SearchAdapter extends BaseAdapter implements Filterable {
    ...
    private final BookmarkDatabase mBookmarkManager;
	...
	public SearchAdapter(Context context, boolean dark, boolean incognito) {
		...
		mBookmarkManager = BookmarkDatabase.getInstance(context.getApplicationContext());
		mAllBookmarks = mBookmarkManager.getLastItems(PreferenceUtils.getUserId(),0,1000);
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (row == null) {
            ...
            holder.mTitle = row.findViewById(R.id.title);
            holder.mUrl = row.findViewById(R.id.url);
            holder.mImage = row.findViewById(R.id.suggestionIcon);
            row.setTag(holder);
        } else {
    ...
    public List<HistoryItem> getSuggestions() {
        ...
        for (int n = 0; n < bookmarkSize && n < maxBookmarks; n++) {
            if (mBookmarks != null && mBookmarks.size() > n) {
	...
	public class SearchFilter extends Filter {
		...
		protected FilterResults performFiltering(CharSequence constraint) {
			...
			mHistory = mDatabaseHandler.findItemsContaining(PreferenceUtils.getUserId(),constraint.toString());
