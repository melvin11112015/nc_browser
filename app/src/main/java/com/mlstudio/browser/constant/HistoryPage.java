...
package com.mlstudio.browser.constant;
...
public class HistoryPage {
	...
	private static final String HEADING = "<!DOCTYPE html><html xmlns=\"http://www.w3.org/1999/xhtml\"><head><meta content=\"en-us\" http-equiv=\"Content-Language\" /><meta content=\"text/html; charset=utf-8\" http-equiv=\"Content-Type\" /><meta name=\"viewport\" content=\"width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=no\"><title>"
			+ MainApp.getContext().getString(R.string.action_history)
	...
	private static final String END = "</div>"
			...
			+" alert(url)"
			+" var x = document.querySelector('a[href='+url+']'); "
	...
	public static String getHistoryPage(String userId,Context context) {
		StringBuilder historyBuilder = new StringBuilder();
		historyBuilder.append(HistoryPage.HEADING);
		List<HistoryItem> historyList = getWebHistory(userId,context);
	...
	private static List<HistoryItem> getWebHistory(String userId,Context context) {
		HistoryDatabase databaseHandler = HistoryDatabase.getInstance(context
				.getApplicationContext());
		return databaseHandler.getLastHundredItems(userId);
	}
}
