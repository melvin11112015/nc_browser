...
public class HttpBytesUploadTaskInner extends AsyncTask<String, Void, String> {
	...
	public HttpBytesUploadTaskInner(OnUploadTaskHandler listener,
									List<NameValuePair> params, byte[] f) {
		this.listener = listener;
		this.params = params;
		this.uploadedBytes = f;
	...
	private String downloadUrl(String postUrl) {
		...
		String attachmentFileName = "upload file";
		String crlf = "\r\n";
		String lineEnd = "\r\n";
		String twoHyphens = "--";
		String boundary = "*****";
		...
		try {
			...
			InputStream fStream = new ByteArrayInputStream(uploadedBytes);
			/* 设置每次写入1024bytes */
			int bufferSize = 1024 * 50;
			byte[] buffer = new byte[bufferSize];
			int length = -1;
			int count = 0;
			int total = uploadedBytes.length;
			/* 从文件读取数据至缓冲区 */
	...
	private void publishProgress(int count, int total) {
		int percent = (int) ((count / (float) total) * 100);
		if (listener != null) {
			listener.onUploadeBytes(count,total);
		}
